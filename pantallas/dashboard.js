...
(function () {
  ...
  D.Screens.Dashboard = function Dashboard({ products, setView, movements, cashOpen }) {
    ...
    return h(React.Fragment, null, [
      h('div', { className:'section-kicker', key:'kicker' }, D.todayLabel),
      h('div', { className:'hero-card', key:'hero' }, [
        h('div', { className:'hero-copy' }, [
          h('div', { className:'section-kicker' }, cashOpen ? 'Turno abierto · Caja 01' : 'Turno cerrado'),
          h('h1', null, 'La caja de hoy empieza con una cifra clara.'),
          h('p', null, 'Consulta el pulso de tu operación y decide el siguiente movimiento sin perder el rastro.'),
          h(Button, { kind:'primary', icon:'arrow', onClick:() => setView('ventas') }, 'Nueva venta'),
        ]),
        h('div', { className:'hero-stamp' }, [h(Icon, { name:'check', size:13 }), 'Registro activo']),
      ]),
      h('div', { className:'kpi-grid', key:'kpis' }, [
        h(Kpi, { label:'Venta del turno', value:'$ 1.846.000', note:'+ 8 operaciones', primary:true }),
        h(Kpi, { label:'Cartera pendiente', value:'$ 2.198.000', note:'3 clientes con saldo' }),
        h(Kpi, { label:'Stock por revisar', value:String(lowStock).padStart(2,'0'), note:'productos bajo atención' }),
        h(Kpi, { label:'Caja esperada', value:'$ 2.026.000', note:'contado + abonos' }),
      ]),
      ...
      h('div', { className:'action-rail', key:'actions' }, [
        ...
        h(Action, { icon:'users', title:'Registrar abono', detail:'Actualizar cartera', onClick:() => setView('clientes') }),
        h(Action, { icon:'box', title:'Revisar stock', detail:lowStock + ' alertas activas', onClick:() => setView('productos') }),
        h(Action, { icon:'cash', title:'Cuadrar caja', detail:'Cierre de turno', onClick:() => setView('caja') }),
      ]),
