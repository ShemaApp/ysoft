...
(function () {
  ...
  D.Screens.ProductsView = function ProductsView({ products, locations = [], activeLocationId, onLocationChange, inventoryBalances = {}, inventoryMovements = [], onAdd, setToast }) {
    const [search, setSearch] = useState(''); const [category, setCategory] = useState('Todos'); const [selectedProductId, setSelectedProductId] = useState('');
    ...
    const activeLocation = locations.find((location) => location.id === activeLocationId) || locations[0] || { id:activeLocationId, name:'Ubicación activa', code:activeLocationId };
    const selectedProduct = products.find((product) => product.id === selectedProductId);
    const currentStock = (product) => Number((inventoryBalances[product.id] && inventoryBalances[product.id][activeLocationId]) ?? (product.stock ?? 0));
    const lowStock = products.filter((product) => currentStock(product) < 8).length;
    const productMovements = selectedProduct ? inventoryMovements.filter((movement) => movement.productId === selectedProduct.id && (!movement.locationId || movement.locationId === activeLocationId)) : [];
    if (selectedProduct) return h(React.Fragment, null, [
      h('div', { className:'section-kicker' }, 'Catálogo · Ficha de producto'), h('h1', { className:'page-title' }, selectedProduct.name),
      h('p', { className:'page-intro' }, 'Consulta el saldo de la ubicación activa y el rastro de movimientos sin editar existencias desde esta pantalla.'),
      h('div', { className:'scope-bar' }, [h('div', { className:'scope-copy' }, [h(Icon, { name:'box', size:17 }), h('div', null, [h('strong', null, activeLocation.name), h('span', null, activeLocation.code + ' · lectura por ubicación')])]), h('select', { className:'input scope-select', value:activeLocationId, onChange:(event) => onLocationChange(event.target.value), 'aria-label':'Ubicación activa' }, locations.map((location) => h('option', { value:location.id, key:location.id }, location.name)))]),
      h('section', { className:'panel product-detail-panel' }, [h('div', { className:'detail-code' }, selectedProduct.code + ' · ' + selectedProduct.category), h('div', { className:'detail-price' }, D.money(selectedProduct.price)), h('div', { className:'detail-stock' }, currentStock(selectedProduct) + ' ' + selectedProduct.unit + ' disponibles en ' + activeLocation.name), h('div', { className:'detail-meta-grid' }, [h('div', null, [h('span', null, 'Unidad de venta'), h('strong', null, selectedProduct.unit)]), h('div', null, [h('span', null, 'Estado'), h(Status, { tone:currentStock(selectedProduct) < 8 ? 'red' : 'green' }, currentStock(selectedProduct) < 8 ? 'Atención' : 'Disponible')])]), h('div', { className:'flow-actions' }, [h(Button, { kind:'secondary', onClick:() => setSelectedProductId('') }, 'Volver al catálogo'), h(Button, { kind:'primary', onClick:() => { onAdd({ ...selectedProduct, stock:currentStock(selectedProduct) }); setToast(selectedProduct.name + ' agregado a la salida.'); } }, 'Agregar a venta')])]),
      h('section', { className:'panel' }, [h('div', { className:'panel-title' }, [h('div', null, [h('h3', null, 'Existencias por ubicación'), h('p', null, 'Lectura resumida · los cambios requieren un movimiento autorizado.')])]), h('div', { className:'inventory-location-list' }, locations.map((location) => { const stock = Number((inventoryBalances[selectedProduct.id] && inventoryBalances[selectedProduct.id][location.id]) ?? (location.id === activeLocationId ? selectedProduct.stock : 0)); return h('div', { className:'inventory-location-row', key:location.id }, [h('div', null, [h('strong', null, location.name), h('span', null, location.code + ' · ' + location.type)]), h('strong', { className:stock < 8 ? 'stock-value low' : 'stock-value' }, stock + ' ' + selectedProduct.unit)]); }))]),
      h('section', { className:'panel' }, [h('div', { className:'panel-title' }, [h('div', null, [h('h3', null, 'Movimientos recientes'), h('p', null, productMovements.length + ' hechos en la ubicación activa')])]), productMovements.length ? h('div', { className:'movement-list' }, productMovements.map((movement) => h('div', { className:'movement-row', key:movement.id }, [h('div', null, [h('strong', null, movement.type), h('span', null, (movement.reason || 'Movimiento registrado') + ' · ' + (movement.time || 'Fecha registrada'))]), h('div', { className:'movement-quantity ' + (Number(movement.quantitySigned) < 0 ? 'negative' : 'positive') }, [(Number(movement.quantitySigned) < 0 ? '−' : '+') + Math.abs(Number(movement.quantitySigned || movement.quantity || 0)), h('span', null, 'saldo ' + Number(movement.stockAfter ?? 0))])]))) : h('div', { className:'empty-state compact-empty' }, [h(Icon, { name:'ledger', size:22 }), h('strong', null, 'Sin movimientos visibles'), h('p', null, 'El historial aparecerá cuando exista un hecho autorizado.')])]),
    ]);
    return h(React.Fragment, null, [
      h('div', { className:'section-kicker' }, 'Catálogo y existencias'), h('h1', { className:'page-title' }, 'Inventario'),
      h('p', { className:'page-intro' }, 'Consulta productos y saldos por ubicación. Las existencias se muestran como proyección; cada cambio necesita un movimiento y un motivo.'),
      h('div', { className:'scope-bar' }, [h('div', { className:'scope-copy' }, [h(Icon, { name:'box', size:17 }), h('div', null, [h('strong', null, activeLocation.name), h('span', null, activeLocation.code + ' · ' + activeLocation.type)])]), h('select', { className:'input scope-select', value:activeLocationId, onChange:(event) => onLocationChange(event.target.value), 'aria-label':'Ubicación activa' }, locations.map((location) => h('option', { value:location.id, key:location.id }, location.name)))]),
      h('div', { className:'inventory-summary' }, [h('div', { className:'inventory-summary-card' }, [h('span', null, 'Productos visibles'), h('strong', null, filtered.length)]), h('div', { className:'inventory-summary-card' }, [h('span', null, 'Bajo atención'), h('strong', { className:lowStock ? 'warning-number' : '' }, lowStock)]), h('div', { className:'inventory-summary-card' }, [h('span', null, 'Alcance'), h('strong', null, activeLocation.code)])]),
      ...
      h('div', { className:'product-grid' }, filtered.map((product) => h('article', { className:'product-card', key:product.id }, [
        h('div', { className:'product-top' }, [h('span', { className:'product-code' }, product.code), h(Status, { tone:currentStock(product) < 8 ? 'red' : currentStock(product) < 15 ? 'orange' : 'green' }, currentStock(product) < 8 ? 'Atención' : 'Activo')]),
        h('button', { className:'product-card-title-button', onClick:() => setSelectedProductId(product.id) }, [h('div', { className:'product-name' }, product.name), h('span', { className:'product-code' }, 'Ver ficha →')]), h('div', { className:'product-price' }, D.money(product.price)), h('div', { className:'product-unit' }, 'por ' + product.unit),
        h('div', { className:'stock-line' }, [h('span', null, 'Existencia en ' + activeLocation.code), h('span', { className:'stock-value' + (currentStock(product) < 8 ? ' low' : '') }, currentStock(product) + ' ' + product.unit)]),
        h('div', { className:'product-card-actions' }, [h(Button, { kind:'secondary', onClick:() => setSelectedProductId(product.id) }, 'Ficha'), h(Button, { kind:'primary', onClick:() => { onAdd({ ...product, stock:currentStock(product) }); setToast(product.name + ' agregado a la salida.'); } }, 'Agregar')]),
      ]))),
    ]);
  };
  ...
  D.Screens.CustomersView = function CustomersView({ customers, registerPayment, setToast, savingOperation }) {
